...
问题在于上下文的墙。Agent 的有效处理容量有硬上限，执行时间一长就会撞上去。但这堵墙可以翻过去。任务分解将大任务切成 Agent 能消化的块，知识持久化让关键信息在会话边界存活。掌握了这些，Agent 可以跨会话、跨天地持续推进一个项目。你从 Agent 的实时对话伙伴变成了任务的设计者和验收者。
